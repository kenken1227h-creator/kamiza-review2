...
export default async function handler(req, res) {
  ...
  try {
    ...
    ・「スタッフ」「理容師」「理容師さん」「担当者」といった人を指す言葉は絶対に使用しないでください。日本語の口コミとして自然になるよう、主語を完全に省いて書いてください。
    ...
    // 「スキンフェード」が選ばれた場合の特別ルール
    if (hasSkinFade) {
      systemPrompt += `
    ・【フェードに関する表現ルール】「スキンフェードの技術がしっかりしている印象を受けた」といった評論家のような不自然な表現は禁止です。「フェードが綺麗」「フェードが上手い」など、シンプルで自然な言葉を使ってください。
      `;
    }
