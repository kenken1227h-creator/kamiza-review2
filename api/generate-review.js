...
export default async function handler(req, res) {
  ...
  try {
    ...
    たとえ指示が日本語で書かれていても、結果の文章に日本語を一切混ぜないでください（※日本語が選択された場合を除く）。

    【絶対的なルール（サクラ感とAIっぽさを消すこと）】
    ・「とても」「本当に」「最近」という言葉（および他言語での同義語）は、わざとらしくなるため極力使用しないでください。
    ・「スタッフ」「理容師」「理容師さん」「担当者」といった人を指す言葉は絶対に使用しないでください。主語を完全に省くか自然な表現にして書いてください。
    ・「結論から言うと」「今回、」等のAI特有の堅苦しい表現は使用禁止。
    ・冷たすぎず、かといって大げさすぎない「中間のトーン」にしてください。
    ・短くてスパッとした一文を交えつつ、冷たい印象になりすぎないよう自然な感想にしてください。
    ...
    if (hasSpeedy) {
      systemPrompt += `
    ・【禁止事項】手際の良さに触れる際、「待ち時間も少なく」や「時間がないときに助かる」といった表現（および他言語での同義語）は絶対に使わないでください。
      `;
    }
    
    if (hasSkinFade) {
      systemPrompt += `
    ・【フェードに関する表現ルール】「スキンフェードの技術がしっかりしている印象を受けた」といった評論家のような不自然な表現は禁止です。「フェードが綺麗」「フェードが上手い」など、シンプルで自然な言葉（および他言語での同義語）を使ってください。
      `;
    }
    ...
    const userPrompt = `メニュー：${menuTexts.join(", ")}\n良かった点：${pointTexts.join(", ")}\n自由記述（一言感想）：${freeText}\n\nこの条件で、絶対にAIが書いたとバレない口コミを生成してください。必ず「${targetLang}」だけで出力してください。`;
